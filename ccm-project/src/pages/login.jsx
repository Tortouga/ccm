...
function Login() {
	...
	const { handleLogin, loggedInUser } = useAuth();
	...
	const onSubmit = async (e) => {
		...
		try {await handleLogin(email, password);navigate("/"); // Redirige après connexion réussie
		} catch (error) {
	...
	return (
		<div className="relative min-h-screen bg-gradient-to-br from-red-900 via-black to-yellow-900 flex items-center justify-center font-serif overflow-hidden">
			...
			<form
				...
				<h2 className="text-4xl font-bold mb-6 text-center text-yellow-300 tracking-widest">🎰 Connexion au Casino</h2>
				<label className="block mb-2 text-sm font-medium text-yellow-200">🎲 Identifiant</label>
				<input type="email"	value={email}	onChange={(e) => setEmail(e.target.value)}	className="w-full mb-4 px-4 py-2 rounded-lg bg-red-950 border border-yellow-400 text-yellow-100 placeholder-yellow-300 focus:outline-none focus:ring-2 focus:ring-yellow-400"	placeholder="ID"	required/>
				<label className="block mb-2 text-sm font-medium text-yellow-200">💰 Mot de passe</label>
				<input
					type="password"
					value={password}
