...
import {useArgent} from "@/context/argentContext";
import { ArgentProvider } from "@/context/argentContext";

export const Roulette = () => {
const { argent, updateArgent } = useArgent();
		...
		const start = () => {
			...
			const animate = (timestamp) => {
				...
				if (t < 1) {
					...
					// ✅ Utilisation de updateArgent du contexte
					let gain = 0;
					if (finalItem.name === "-50") gain = -50;
					else if (finalItem.name === "-25") gain = -25;
					else if (finalItem.name === "+35") gain = +35;
					else if (finalItem.name === "+75") gain = +75;
					else if (finalItem.name === "+200") gain = +200;
					else if (finalItem.name === "-500") gain = -500;

					updateArgent(gain); // 🧠 On met à jour le wallet
					setPendingAnim(null);
				}
			};
